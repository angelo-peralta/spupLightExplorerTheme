...
{block name="page"}
	{if !$currentContext}
		<link rel="stylesheet" href="{$baseUrl|escape}/plugins/themes/spupLightExplorerTheme/resources/root-reset.css">
		<div class="spup-reset-shell">
			<aside class="spup-reset-identity" aria-label="{translate|escape key='plugins.themes.spupLightExplorerTheme.footer.institution'}">
				{if $displayPageHeaderLogo}<img src="{$sitePublicFilesDir}/{$displayPageHeaderLogo.uploadName|escape:'url'}" alt="">{/if}
				<p class="spup-reset-identity__publisher">{translate key="plugins.themes.spupLightExplorerTheme.footer.institution"}</p>
				<p class="spup-reset-identity__title">{$siteTitle|escape}</p>
				<p class="spup-reset-identity__subtitle">{$spupRootOptions.networkSubtitle|escape}</p>
			</aside>
			<div class="spup-reset-content">
	{/if}
